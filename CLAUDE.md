...
├── plugin/               ← Claude Code Plugin 빌드 소스
...
└── plugin.json           ← 플러그인 메타데이터 (name, version, 컴포넌트 경로)

scripts/
└── build-plugin.mjs      ← Claude Code Plugin 빌드 (dist/claude-plugin/ 생성)
...
npm run dev            # 워치 모드
npm run build          # Obsidian 플러그인 + CLI + Claude Code Plugin 모두 빌드
npm run build:obsidian # Obsidian 플러그인만 빌드
npm run build:cli      # CLI만 빌드
npm run build:plugin   # Claude Code Plugin만 빌드 (dist/claude-plugin/)
...
npx oh-my-til plugin-path                       # Claude Code Plugin 디렉토리 경로 출력
