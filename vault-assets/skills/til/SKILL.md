...
- **Phase 1 시작 전**: `til_list`로 기존 TIL 목록을 확인하여 동일/유사 주제가 있으면 심화 학습 모드로 전환
- **Phase 2 중**: `til_get_context`로 주제 관련 기존 TIL과 백로그를 파악하여 wikilink 후보 확보
...
1. `til_list` 또는 `til_get_context`로 동일/유사 주제의 기존 TIL이 있는지 확인한다
2. **기존 TIL이 있으면 (심화 학습 모드)**:
   - 기존 TIL 파일을 읽어 이미 다룬 내용을 파악한다
   - "기존 TIL이 있습니다. 어떤 부분을 더 깊이 알고 싶나요?"라고 안내한다
   - 사용자 응답을 바탕으로 해당 영역을 웹 검색/문서로 조사한다
3. **기존 TIL이 없으면 (새 학습 모드)**: 웹 검색과 문서를 통해 주제를 조사한다
...
4. 사용자가 충분히 이해했다고 판단되면 Phase 3으로 넘어간다
5. 사용자가 "저장해줘", "정리해줘", "TIL로 만들어줘" 등을 말하면 Phase 3으로 전환한다
...
- Wikilink는 항상 `[[til/{카테고리}/{slug}|표시명]]` 경로 기반 형식을 사용한다. 기존 TIL과 백로그 항목은 본문에서 바로 링크하고, 존재하지 않는 개념은 저장 시 사용자에게 확인 후 관련 노트 섹션에 추가한다
- `[[개념]]` 같은 짧은 형식 사용 금지 — 항상 경로 기반 `[[til/{카테고리}/{slug}|표시명]]` 형식만 사용한다
